		...
		png_byte colortype, char* file_name)
{
	...
	if (!fp)
		panic("Write Error: File %s could not be opened for writing", file_name);
...
void colourCheck(imgpng *img) {
	if (png_get_color_type(img->png_ptr, img->info) == PNG_COLOR_TYPE_RGB)
		panic("Processing Error: input file is PNG_COLOR_TYPE_RGB but must be PNG_COLOR_TYPE_RGBA "
					 "(lacks the alpha channel)");

	if (png_get_color_type(img->png_ptr, img->info) != PNG_COLOR_TYPE_RGBA)
		panic("Processing Error: color_type of input file must be PNG_COLOR_TYPE_RGBA (%d) (is %d)",
